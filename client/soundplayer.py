...
class SoundPlayer:
    ...
    '''
    @staticmethod
    def playTone(frequencies, duration, volume = 0.05, blocking = False, device = 0):
        '''
        ...
        if blocking:
            SoundPlayer._emit(frequencies, duration, volume, device)
        else:
            thread.start_new_thread(SoundPlayer._emit, (frequencies, duration, volume, device))
    ...
    @staticmethod
    def _emit(frequencies, duration, volume, device):
        s = " "
        for f in frequencies:
            s += "sin " + str(f) + " "
        cmd = "AUDIODEV=hw:" + str(device) + " play -q -n synth " + str(duration) + \
            s + "vol " + str(volume) + " 2> /dev/null" 
        os.system(cmd)
